...
class XhsDbStoreImplement(AbstractStore):
    async def store_content(self, content_item: Dict):
        ...
        if not note_detail:
            ...
            content_item["create_time"] = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            await add_new_content(content_item)
            utils.logger.info(f"新的xhs笔记入库成功：{content_item.get('note_id')}")
        else:
            content_item["create_time"] = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            await update_content_by_content_id(note_id, content_item=content_item)
            utils.logger.info(f"旧的xhs笔记更新成功：{content_item.get('note_id')}")
